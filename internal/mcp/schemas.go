...
func ToolSchemas() []ToolSchema {
	return []ToolSchema{
		...
		toolSchema("get_pvc_restore", "Get PVCRestore status", `{"type":"object","properties":{"namespace":{"type":"string"},"name":{"type":"string"}},"required":["namespace","name"]}`),
		...
		toolSchema("get_snapshot_download_url", "Mint a signed Backrest download URL. Default mode=restore schedules Restore (visible in UI). mode=stream dumps from indexed snapshot only.", `{"type":"object","properties":{"namespace":{"type":"string"},"name":{"type":"string","description":"BackupRepository name"},"snapshot_id":{"type":"string"},"path":{"type":"string","description":"Path inside snapshot; default /"},"plan_id":{"type":"string"},"mode":{"type":"string","enum":["restore","stream"]},"public_base_url":{"type":"string"},"cluster_namespace":{"type":"string"},"cluster_name":{"type":"string"}},"required":["namespace","name","snapshot_id"]}`),
		toolSchema("create_snapshot_download", "Create SnapshotDownload CR; waits until status.downloadURL is Ready (mode=restore → visible in Backrest UI).", `{"type":"object","properties":{"namespace":{"type":"string"},"repository_name":{"type":"string"},"repository_namespace":{"type":"string"},"snapshot_id":{"type":"string"},"path":{"type":"string"},"plan_id":{"type":"string"},"mode":{"type":"string","enum":["restore","stream"]},"public_base_url":{"type":"string"},"download_name":{"type":"string"},"wait_seconds":{"type":"integer"}},"required":["namespace","repository_name","snapshot_id"]}`),
		toolSchema("get_snapshot_download", "Get SnapshotDownload status including downloadURL", `{"type":"object","properties":{"namespace":{"type":"string"},"name":{"type":"string"}},"required":["namespace","name"]}`),
